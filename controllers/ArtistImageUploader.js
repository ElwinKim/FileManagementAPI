...
exports.resizeImage = catchAsync(async (req, res, next) => {
  ...
  if (req.params.id) {
    req.file.filename = `artist_${req.params.id}.jpeg`;
  } else {
    next();
  }
...
exports.createNewImage = catchAsync(async (req, res, next) => {
  ...
  const containerName = 'artistimages';
...
exports.deleteImage = catchAsync(async (req, res, next) => {
  ...
  if (req.query) {
    ...
    for (var i = 0; i < queryLength; i++) {
      const blobName = `artist_${Object.keys(queryObj)[i]}.jpeg`;
      const blockBlobClient = containerClient.getBlockBlobClient(blobName);
    ...
    const blobName = `artist_${req.params.id}.jpeg`;
    const blockBlobClient = containerClient.getBlockBlobClient(blobName);
    await blockBlobClient.delete().catch(function (error) {
      console.log(`Cannot find file! \n error: ${error}`);
    });
